...
const HS_LANDSCAPE = {
  ...
  about:     { id: "hs-about",     x: 0.8220, y: 0.6600, w: 0.0660, h: 0.0360, skew: 0.0, rot: -4.0 } // ✅ Final About values
};
...
window.layout = function layout() {
  ...
  if (IS_MOBILE) {
    ...
    const scaleW = vw / iw;
    const scale = scaleW * 0.88; // ✅ zoom level
...
let step = 0.002;                  // 👈 movement/scale step
let angleStep = 1;                 // 👈 rotation/skew step (degrees)
